<template>
    ...
    >
        ...
        <a-form-item label="选择违约原因">
            <a-select
                v-decorator="['violateReason',
          {
            rules: [
              { required: true, message: '必须选择一条违约原因！' },
            ],
          },
        ]"
                placeholder="请选择违约原因"
        ...
        handleSubmit(e) {
            ...
            this.form.validateFields((err, values) => {
                if (!err) {
                    ...
                    axios.post("http://localhost:8081/violate/apply",{
                        name:values.name,
                        ...
                        if(res.data.msg=="查询不到该客户")
                        {
                            ...
                            this.submitMessage = "  已提交客户 "+values.name+" 的违约申请单 !";
                        }
                    })
                }
            });
