...
export default {
    ...
    computed:{
        computedData(){
            if(this.mySearchValue) {
                return this.VQ_table_data_1.filter(item =>{
                        return Object.keys(item).some(key => {
                            return String(item[key]).toLowerCase().indexOf(this.mySearchValue) > -1
                        })
                    }
                )
            }else return this.VQ_table_data_1
        }
    },
    mounted(){
        axios.get("http://localhost:8081/violate/af",{
            ...
            for(var i=0;i<res.data.data.length;i++){
                this.VQ_table_data_1[i].name=res.data.data[i].name;
                this.VQ_table_data_1[i].level=res.data.data[i].level;
                this.VQ_table_data_1[i].status=res.data.data[i].defaultState;
                this.VQ_table_data_1[i].verifyStatus=res.data.data[i].verifyStatus;
                this.VQ_table_data_1[i].violateReason=res.data.data[i].violateReason;
                this.VQ_table_data_1[i].respawnReason=res.data.data[i].respawnReason;
                this.VQ_table_data_1[i].violateGravity=res.data.data[i].violateGravity;
                this.VQ_table_data_1[i].confirmedPerson=res.data.data[i].confirmedPerson;
                this.VQ_table_data_1[i].applyTime=res.data.data[i].applyTime;
                this.VQ_table_data_1[i].verifyTime=res.data.data[i].verifyTime;
            }
            this.load();
        })
    },
    methods:{
        handleViolate(index, row) {
            // this.VQ_table_data_1.splice(index, 1)
            this.$router.push({
                name: 'ViolateVerify', // 要跳转的路径的 name,可在 router 文件夹下的 index.js 文件内找
                //params: params
                params: {
                    nameData:this.VQ_table_data_1[index].name
                }
            })
            console.log("违约查询表格(1)组件发送了值："+this.VQ_table_data_1[index].name)
        },
        sleep(milliSeconds) {
